...
func NewAliLS() logs2.Logger {
	alils := new(aliLSWriter)
	alils.Level = logs2.LevelTrace
...
func (c *aliLSWriter) Init(config string) error {
	...
	if len(c.Formatter) > 0 {
		fmtr, ok := logs2.GetFormatter(c.Formatter)
		if !ok {
...
func (c *aliLSWriter) Format(lm *logs2.LogMsg) string {
	return lm.OldStyleFormat()
}

func (c *aliLSWriter) SetFormatter(f logs2.LogFormatter) {
	c.formatter = f
}

// WriteMsg writes a message in connection.
// If connection is down, try to re-connect.
func (c *aliLSWriter) WriteMsg(lm *logs2.LogMsg) error {
	if lm.Level > c.Level {
		return nil
	}
...
func init() {
	logs2.Register(logs2.AdapterAliLS, NewAliLS)
}
